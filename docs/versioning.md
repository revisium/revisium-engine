...
[Consistency and ChangeSet Contract](consistency.md) is the normative VE-011
target, while the [Partial Commit Design](design/partial-commit.md) is a
non-normative future design for an unaccepted PDR-008 and has no runtime API.
...
### Target singleton ChangeSet lifecycle (not implemented)

VE-011 defines exactly one computed ChangeSet for every structurally valid
Draft, including an empty projection when Head and Draft are semantically
equal. It is not a persisted staging object. The ChangeSet ID belongs to the
Draft Revision and remains stable through semantic edits, physical
copy-on-write, single-item Discard, and Discard all. Full Commit promotes the
old Draft to Head and creates a new Draft, so the new Draft has a new ChangeSet
ID.
...
Target Discard all retains the Draft Revision and restores its complete state
to Head. Target full Commit creates an immutable snapshot of the complete
Draft, not a delta-only Revision. The exact semantic projection, validation,
outcome, and compatibility rules are owned by
[the normative contract](consistency.md).
...
This section documents the present `createRevision` mechanics. The target
...
This section documents the present `revertChanges` mechanics. The target
...
field as their no-changes gate. The VE-011 target computes new ChangeSet reads
and mutations from the canonical semantic Head-to-Draft delta instead; it does
not silently repair this stored cache. The complete four-cell compatibility
matrix is normative in
